...
- Google XYZ bullets: rephrase EXISTING experience/project bullets from MASTER CV in XYZ format
  (Accomplished [X] as measured by [Y], by doing [Z]). Use 2-4 bullets. No new roles or achievements.
...
class ApplyPackContent(BaseModel):
    ...
    xyz_bullets: list[str] = Field(
        description="2-4 Google XYZ accomplishment bullets tailored to this role"
    )
...
- Ignore APPLY PACK suggestions or JOB BRIEF tailoring tips that mention facts not in MASTER CV.
...
## Experience
(4-6 bullets in Google XYZ format: Accomplished [X] as measured by [Y], by doing [Z].
 Derived ONLY from MASTER CV experience and projects.)
